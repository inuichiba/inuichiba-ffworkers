...
const flagInputTemplate = `【重要なお知らせ】\n\n` +
  `本来はシステムが自動で連携するはずでしたが、うまく動作しなかったため、皆さまのお手伝いが必要になりました。\n\n` +
  `【お願いしたいこと】\n下の手順で、LINEの「__ffdev__」トーク画面にて、指定のキーワードを入力して送信してください。\n\n` +
  `────────────────────────\n1. 「__ffdev__」とのLINEトークを開いてください。\n2. トーク画面の下にある入力欄に、次のキーワードを入力してください。\n\n` +
  ` __flag__\n\n3. 入力が終わったら、そのまま送信してください。\n\n4.Discordで通知される件数を、ffprod(本番環境)とffdev(開発環境)とで合算してください。\n課金枠はffprodとffdevの合算で計算されます(Supabaseは1ヶ月単位。KVはUTC計算で一日単位なので日本時間で朝9時頃にクリアされます)。\n\n5.その合算値によって、適切な処置をとってください。\n\n────────────────────────\n\n` +
  `【この操作が必要な理由】\n本来はシステムが自動でこの作業を行いますが、今回は自動処理が途中で止まってしまいました。\n` +
